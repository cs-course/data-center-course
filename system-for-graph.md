...
- 本实验要求你**亲手实现一个 out-of-core 的二阶游走采样器**，在中等规模图（约 $10^5$–$10^6$ 边，如 PPI / LiveJournal 子集）上体会状态空间与 I/O 优化的权衡。
...
- **嵌入质量**：链接预测 AUC（取 20% 边作验证集）。
- **评分构成**：方法报告与可复现性 **≥ 70%**，排行榜排名占比 **≤ 30%**；设 baseline 分数线，认真完成不挂科。
...
- 开始日期 / 提交日期：按 **2026 秋季学期**教学日历另行通知。
...
- 作为**应用拓展**，感兴趣的同学可在主实验之外选做，体会表示学习在 KG 上的落地。
- 知识图谱表示：**三元组（h,r,t）**；链接预测即已知头/尾实体与关系，预测缺失的尾/头实体。
- 赛题数据、格式、指标详见[官网](https://tianchi.aliyun.com/competition/entrance/532419/)；原提交方式（官网 tsv + 微助教 Notebook）不变。
