...
#### node2vec：二阶转移概率
这一页讲的是 node2vec 随机游走最核心的设计：**二阶游走**，区别于普通的一阶随机游走。

- 回顾普通随机游走（**一阶**）

普通图随机游走：走到当前节点 $v_i$，下一步只看 $v_i$ 的邻居，按边权重挑下一个点。
> 状态只由**当前节点**决定，不记得上一步从哪来。
缺点：没法灵活控制游走是偏向“深度优先DFS”还是“广度优先BFS”。

---

- node2vec 的**二阶**：状态是一对节点 $(t, v_i)$

  - $t$：前驱节点（上一步来的节点）
  - $v_i$：当前所在节点

> 游走做选择时，**不仅看现在在哪，还记得刚才从哪个点过来**，所以叫**二阶转移**。
...
$$
P(v_{i+1}=x \mid v_i, v_{i-1}=t) = \frac{\pi_{v_i x}}{\sum_{y\in N(v_i)}\pi_{v_i y}},\quad \pi_{v_i x}=\alpha_{pq}(t,x)\cdot w_{v_i x}
$$

- $w_{v_i x}$：节点 $v_i$ 和邻居 $x$ 之间原始边权重
- $\alpha_{pq}(t,x)$：矫正系数，由 $t$（来路节点）和候选邻居 $x$ 的最短距离 $d_{tx}$ 决定
...
$t$ 是上一站，$v_i$ 是当前站，$x$ 是候选下一站：

1. $d_{tx}=0$：$x$ 就是来路节点 $t$，也就是走回头路。权重乘 $(1/p)$。
   - $p$ 越大，越不喜欢走回上一个点，抑制回头。
2. $d_{tx}=1$：$x$ 和来路节点 $t$ 直接相连。相当于就在附近小范围逛，系数=1，正常概率。
3. $d_{tx}=2$：$x$ 和来路节点 $t$ 不直接相连，只能通过当前点 $v_i$ 连通，相当于向外探索更远节点。权重乘 $(1/q)$。
   - $(q>1)$：压低向外探索概率 → **偏向BFS**，聚焦局部邻居；
   - $(q<1)$：鼓励走远一点 → **偏向DFS**，往图深处探索。

<!-- 一句话理解 p 和 q

- $p$：**返回参数**，控制有多大概率原路折返；
- $q$：**进出参数**，控制是原地打转（BFS）还是往深处走（DFS）。 
 -->
...
一阶游走状态数 = 节点数量 $O(|V|)$；
二阶游走状态是 $(t,v_i)$ 节点对，最多 $O(|E|)$，和图的边数相当。

> 好处：只用两个超参，就能同时兼顾BFS（捕捉局部结构）和DFS（捕捉全局社区结构）；
> 代价：需要维护更多状态，计算开销比简单随机游走更大。

#### 价值：蕴含微观结构

通过这种带记忆的二阶随机游走，生成节点序列，再丢给word2vec训练，得到节点向量 embedding。向量同时保留图的局部近邻关系 + 社区结构。
