...
![bg right fit](images/c-func-callstack.svg)

- **值传递**：形参是实参的**副本**，函数内修改不影响外部
- **传地址**：通过指针直接操作原变量（第8章详解）
- `scanf("%d", &x)` 正是利用地址传递把输入写回 `x`
...
![bg right fit](images/c-func-factorial.svg)

- 每次调用规模缩小（`fact(n)` → `fact(n-1)`）
- 遇到终止条件 `n == 0 || n == 1` 开始**回溯**求值
- 调用栈深度与 `n` 成正比，过深时可能栈溢出
...
- **模块化设计**：自顶向下，逐步求精，一个函数一个任务
- **函数三步**：定义 → 声明（原型） → 调用
- **参数传递**：C 语言只有**值传递**，要修改实参需传指针
- **存储类型**：`auto`（栈）、`extern`（全局）、`static`（持久化/限制作用域）、`register`（建议寄存器）
- **递归三要素**：递归条件、规模缩小、终止条件
- **递归优化**：记忆化搜索、尾递归、迭代替代
- **经典问题**：阶乘、斐波那契、汉诺塔、整数分划、八皇后
- **多文件**：`.h` 声明 + `.c` 实现，用 `#ifndef` 防止重复包含
