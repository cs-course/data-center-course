...
![bg right fit](images/c-basic-types.svg)

C 的数据类型分为四大类：

- **基本类型**：整型、浮点型、字符型、布尔型
- **构造类型**：数组、结构、联合、枚举
- **指针类型**：指向各种数据的地址（第8章详述）
- **空类型** `void`：无返回值或空类型
...
> **注**：示例中的 `0b1100` 是二进制字面量，C23 才纳入标准；当前以 `-std=c11` 为基准时属于 GCC/Clang 扩展（可用 `0xC & 0xA` 等十六进制写法替代）。
...
- **字符集与标识符**：C 语言基本符号、命名规则
- **数据类型**：整型（char/short/int/long/long long）、浮点型（float/double）、布尔型（bool）
- **常量与变量**：字面常量、`#define`、`const`；变量先定义后使用
- **运算符**：算术、关系、逻辑、赋值、条件、逗号、sizeof
- **位运算**：与/或/异或/取反/移位——掩码、置位、清位的经典模式
- **类型转换**：隐式（低→高）、显式（强制）、溢出风险
- **优先级**：拿不准就加括号
