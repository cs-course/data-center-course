...
![bg right fit](images/c-stdio-format.svg)
...
- **宽度** 最小输出位数
- **精度** `.n` 小数位数或字符串最大长度
- **转换符** `d/f/c/s/x` 等决定数据类型

记不住顺序时，记住口诀：**% 标志 宽度 .精度 长度 类型**。
...
- **字符 I/O**：`putchar`/`getchar`，注意 `getchar` 返回 `int` 和缓冲区特性
- **printf**：格式控制符语法 `%[标志][宽度][.精度][长度]类型`
- **scanf**：参数必须是地址（`&`），注意 `%lf` 读 double、`%c` 前加空格
- **宽度与精度**：`%10d`、`%-10d`、`%010d`、`%.2f`、`%10.2f`
- **安全输入**：用 `fgets` 替代 `gets`，用 `%99s` 限制 `scanf` 的 `%s` 长度
- **字符串 I/O**：`sprintf`/`snprintf`（格式化写入字符串）、`sscanf`（从字符串读取）
- **返回值**：`printf` 返回输出字符数，`scanf` 返回成功赋值项数
