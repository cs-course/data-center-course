...
在数组长度已知且不变化的场景中，建议提前为数组长度定义常量

```c
#define STU_COUNT 3 // 宏定义长度

struct STUDENT stu[STU_COUNT] = {
    {1, "Tom", 'M', {2006,12,20}, 90,83,72,82},
    {2, "Nick", 'M', {2007,05,01}, 78,92,88,78},
    {3, "Sue", 'F', {2007,07,10}, 89,72,98,66}
};
...
// 遍历找结尾值计算长度
size_t get_stu_arr_len(const Student *arr) {
    size_t len = 0;
    // 遍历直到 id 为 -1（结尾）
    while (arr[len].id != -1) {
        len++;
...
int main() {
    // 初始化最后一个元素为结尾值（id=-1）
    Student stu_arr[] = {{1, "Alice"}, {2, "Bob"}, {3, "Charlie"}, {-1, ""}};
    printf("元素个数：%zu\n", get_stu_arr_len(stu_arr)); // 输出 3
    return 0;
}
```
...
int main() {
    Student stu_arr[] = {{1, "Alice"}, {2, "Bob"}, {3, "Charlie"}};
    // 数组末尾指针 - 数组起始指针 = 元素个数
    size_t len = (&stu_arr)[1] - stu_arr;
    printf("元素个数：%zu\n", len); // 输出 3
    return 0;
}
```
...
```c
struct GPIO_Reg {
    unsigned pin0:1;  // 第0位状态
    unsigned pin1:1;  // 第1位状态
    unsigned     :2;  // 硬件保留位
    unsigned mode:2;  // 工作模式设置
};
```

- **精确匹配硬件布局**：字段对应硬件寄存器的每一位
- **简化访问代码**：直接访问字段，无需位操作宏
- **代码可读性强**：字段名明确表达功能
- **防止误操作**：自动屏蔽未使用位
...
```c
struct TCP_Header {
    unsigned src_port:16;   // 源端口
    unsigned dst_port:16;   // 目标端口
    unsigned seq_num:32;    // 序列号
    unsigned ack_num:32;    // 确认号
    unsigned data_offset:4; // 数据偏移
    unsigned flags:12;      // 控制标志
};
```

- **协议精确实现**：严格按协议规范定义字段宽度
- **自动打包/解包**：编译器处理位级布局
- **减少手动位操作**：避免易错的位运算
- **跨平台一致性**：确保不同系统解析相同

<!-- 如果以后手搓网络协议，从事网络安全或者运维管理需要拆包分析 -->
...
```c
struct PackedData {
    unsigned sensor_a:5;  // 传感器A (5位)
    unsigned         :3;  // 填充到字节边界
    unsigned sensor_b:6;  // 传感器B (6位)
    unsigned         :2;  // 填充对齐
};
```
...
- **内存紧凑布局**：精确控制字段间填充
- **优化存储效率**：减少空洞，节约内存
- **强制对齐要求**：使用`:b`强制字节/字对齐
- **传输优化**：减少网络/存储空间占用

<!-- 若以后参加大学生创新创业鼓捣单片机和树莓派、Arduino -->
...
```c
struct FileFlags {
    unsigned read_only:1;   // 只读标志
    unsigned    hidden:1;   // 隐藏文件
    unsigned    system:1;   // 系统文件
    unsigned          :1;   // 保留位
    unsigned   archive:1;   // 归档标志
    unsigned          :3;   // 未来扩展
};
```
...
- **头指针head**: 存放一个地址，指向第一个元素
- **结点**: 链表中的元素，为结构类型，包括
  - **数据域data**，用户数据
  - **指针域next**，下一结点的地址
- **链尾tail**: 最后一个元素，指针域为`NULL`
...
![bg right fit](images/stack-vs-queue.png)
